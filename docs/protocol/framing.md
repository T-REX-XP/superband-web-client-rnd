...
Notify callbacks may fragment frames across MTU boundaries. Reassemble by reading `dataLength` after seeing `0xCD`, then waiting for `3 + dataLength` bytes (`PacketAssembler` in `src/js/protocol.js`).
