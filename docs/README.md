...
Documentation for the SuperBand electronic badge BLE stack from RnD investigation of **SuperBand 2.1.25** (`com.legend.smartwatch.electronicbadge.android`), and for the Web Bluetooth clients in [`../web/`](../web/) (manager) and [`../tools/debug-console/`](../tools/debug-console/) (debug console).
...
| [Web client](web-console.md) | UI guide for `web/` manager and `tools/debug-console/` console |
...
- **Manager UI:** `cd web && bun install && bun run dev` → http://localhost:8787
- **GitHub Pages:** push `web/` to `main`/`master` (see `.github/workflows/pages.yml`)
